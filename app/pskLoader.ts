...
export class PSKLoader extends THREE.Loader {
  ...
  loadAsync(Options, onProgress) {
    return new Promise(function (resolve, reject) {
      this.load(Options, resolve, onProgress, reject);
    });
  }
      ...
      function (data) {
        try {
          ...
          for (let i = 0; i < RawBones.length; i++) {
            ...
            if (
              i == RawBones.length - 1 ||
              RawBones[i + 1].PointIndex != RawBones[i].PointIndex
            ) {
          ...
          for (let i = 0; i < Wedges.length; i++) {
            posAttr.push(
              Points[Wedges[i].PointIndex].x,
              Points[Wedges[i].PointIndex].y,
              Points[Wedges[i].PointIndex].z
            );
          ...
          for (let i = 0; i < Faces.length; i++) {
            ...
            if (indxMat[Faces[i].MaterialIndex] == undefined)
              indxMat[Faces[i].MaterialIndex] = { count: 0 };
          ...
          geometry.setAttribute(
            "position",
            new THREE.BufferAttribute(new Float32Array(posAttr), 3)
          );
          geometry.setAttribute(
            "uv",
          ...
          for (let i = 0; i < Material.length; i++) {
            ...
            const material = loader.loadAsync(
              `${resourcePath}/${Material[i].Name}.json`
            );
            PromiseLoaders.push(material);
          }

          //Resolve materials
          Promise.all(PromiseLoaders).then(
            (materials) => {
              for (let i = 0; i < materials.length; i++) {
                textures.push(scope.parseMaterial(materials[i]));
              }
              onLoad({ geometry, textures, skeleton: scope.Skeleton });
            },
            (error) => {
              console.error(error);
            }
  ...
  parseMaterial(data) {
    ...
    const result = Object.keys(textures).reduce((acc, key) => {
      acc[key] = textures[key].split(".")[1] + ".png";
      return acc;
    }, {});
  ...
  HeaderChunk(data) {
    const id = THREE.LoaderUtils.decodeText(
      new Uint8Array(data, this.LastByte, HeaderBytes.ChunkID)
    );
    ...
    if (size < 65536) {
      for (let i = 0; i < size; i++) {
        Wedges[i] = {
          ...
          /*Padding: 0,*/ U: 0,
          V: 0,
          MaterialIndex: 0 /*, Reserved: 0, Padding2: 0*/,
        };
  ...
  ReadMaterial(size, data) {
    ...
    for (let i = 0; i < size; i++) {
      ...
      mat[i].Name = THREE.LoaderUtils.decodeText(
        new Uint8Array(data, this.LastByte, MattBytes.Name)
      ).split("\x00")[0];
  ...
  ReadBones(size, data) {
    ...
    for (let i = 0; i < size; i++) {
      bones[i] = {};
      bones[i].Name = THREE.LoaderUtils.decodeText(
        new Uint8Array(data, this.LastByte, BoneBytes.Name)
      ).split("\x00")[0];
